package by.epam.training.helper.constant;

public class ErrorStatus {

	public static final String LOGIN_EXISTS = "login_exist";
	public static final String EMAIL_EXISTS = "email_exist";
	public static final String LOGIN_EMAIL_EXISTS = "login_email_exests";
	public static final String ERROR_SIGN_UP = "sign_up_error";
	public static final String INVALID_NAME = "invalid_name";
	public static final String INVALID_EMAIL = "invalid_email";
	public static final String INVALID_SURNAME = "invalid_surname";
	public static final String INVALID_PASSWORD = "invalid_password";
	public static final String INVALID_LOGIN = "invalid_login";
	public static final String INVALID_ID = "invalid_login";
	public static final String ERROR_USER_NOT_FOUND = "User_not_found";
	public static final String ERROR_PASSWORD_NOT_EQUALS = "The password in the database does not match what the user entered";
	public static final String USER_NOT_LOCK = "user_not_lock";

}

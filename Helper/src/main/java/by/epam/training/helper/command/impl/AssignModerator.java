			...
			try{
				...
				if(user.getId() != userId){
					if(Validation.isValidationId(userId)){
						userService.assignModeratorOrUser(userId, MODERATOR_ROLE);
						response.sendRedirect(Url.SHOW_ALL_USER_WITH_MESSAGE + SuccessMessage.MODERATOR_ROLE);
					}else{
					...
					response.sendRedirect(Url.SHOW_ALL_USER_WITH_MESSAGE + ErrorMessage.ERROR_USER_ASSIGN_USER_HIMSELF);
				}
